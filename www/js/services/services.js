...
ledermodule.service('Sources', function() {
  // Might use a resource here that returns a JSON array

  // Some fake testing data sources[1].title
  var sources = [
	    { title: 'Maya Angelou', id: 1, indivSources: 
    	[
		    {
		        "age": 0, 
		        "id": "benhoen", 
	    ...
	    { title: 'Downtown Crossing', id: 2, indivSources: [] },
	    { title: 'Penthouses', id: 3, indivSources: [] },
	    { title: 'Lucky Numbers', id: 4, indivSources: [] },
	    { title: 'Luxury Auctions', id: 5, indivSources: [] },
	    { title: 'Grammar', id: 6, indivSources: [] }
  ];
  ...
  return {
	    ...
	    getSourcesForProject: function(projectId) {
			var sources = this.all();
			for (var i=0; i<sources.length; i++) {
				if (sources[i].id == projectId) {
					return sources[i].indivSources;
				}
			}
			return null;
	    },

	   getSourceText: function(sourceID) {
	   		//get text of source 
			var sources = this.all();
			for (var i=0; i<sources.length; i++) {
				for (var j=0; j<sources[i].indivSources.length; j++) {	
					if (sources[i].indivSources[j].id == sourceID) {
						return sources[i].indivSources[j].snippet;
					}
				}
			}
			return null;
